...
class LogProcessorApp:
    ...
    def __init__(self, root):
        ...
        self.azure_host_service_var = tk.StringVar(value="batch")
    ...
    def create_widgets(self):
        ...
        self.azure_host_batch_radio = ttk.Radiobutton(
            hosting_frame,
            text="Batch endpoint (daily)",
            variable=self.azure_host_service_var,
            value="batch",
            command=self.on_hosting_mode_change,
        )
        self.azure_host_batch_radio.grid(row=5, column=2, columnspan=2, sticky="w", padx=5, pady=5)

        self.azure_host_sub_label = ttk.Label(hosting_frame, text="Azure Host Sub ID:")
        self.azure_host_sub_label.grid(row=6, column=0, sticky="w", padx=5, pady=5)
        self.azure_host_sub_entry = ttk.Entry(hosting_frame, textvariable=self.azure_host_sub_var, width=30)
        self.azure_host_sub_entry.grid(row=6, column=1, sticky="ew", padx=5, pady=5)

        self.azure_host_tenant_label = ttk.Label(hosting_frame, text="Azure Host Tenant:")
        self.azure_host_tenant_label.grid(row=6, column=2, sticky="w", padx=5, pady=5)
        self.azure_host_tenant_entry = ttk.Entry(hosting_frame, textvariable=self.azure_host_tenant_var, width=20)
        self.azure_host_tenant_entry.grid(row=6, column=3, sticky="ew", padx=5, pady=5)

        self.azure_host_compute_label = ttk.Label(hosting_frame, text="Azure Host Compute:")
        self.azure_host_compute_label.grid(row=7, column=0, sticky="w", padx=5, pady=5)
        ...
        self.azure_host_compute_combo.grid(row=7, column=1, sticky="w", padx=5, pady=5)
        ...
        self.azure_host_instance_label.grid(row=7, column=2, sticky="w", padx=5, pady=5)
        ...
        self.azure_host_instance_combo.grid(row=7, column=3, sticky="ew", padx=5, pady=5)

        self.azure_batch_input_label = ttk.Label(hosting_frame, text="Batch Input URI:")
        self.azure_batch_input_label.grid(row=8, column=0, sticky="w", padx=5, pady=5)
        self.azure_batch_input_entry = ttk.Entry(
            hosting_frame,
            textvariable=self.azure_batch_input_var,
            width=30,
        )
        self.azure_batch_input_entry.grid(row=8, column=1, columnspan=3, sticky="ew", padx=5, pady=5)
        ...
        self.azure_batch_note_label = ttk.Label(
            hosting_frame,
            text="Batch schedules need an Azure-accessible file or folder URI, such as azureml://... or https://... .",
            wraplength=460,
            justify="left",
        )
    ...
    def on_hosting_mode_change(self):
        ...
        is_batch = is_azure and self.azure_host_service_var.get().strip() == "batch"
        azure_widgets = [
            self.azure_host_service_label,
            self.azure_host_online_radio,
            self.azure_host_batch_radio,
        ...
        batch_widgets = [
            self.azure_batch_input_label,
            self.azure_batch_input_entry,
            self.azure_batch_time_label,
            self.azure_batch_time_entry,
            self.azure_batch_timezone_label,
            self.azure_batch_timezone_combo,
            self.azure_batch_note_label,
        ]
...
{"source_file":"sample.csv","row_index":1,"errorMessage":"timeout while opening socket","prediction":"Error"}"""
        ...
        health_action = (
            ...
            if health_url and not is_azure_batch else ""
        )
...
"""
    ...
    def start_hosting_thread(self):
        ...
        if hosting_mode == "azure":
            ...
            azure_service = clean_optional_string(self.azure_host_service_var.get()) or "batch"
    ...
    def run_azure_hosting(
        ...
        service_kind = clean_optional_string(azure_service) or "batch"
        ...
        try:
            ...
            for instance_type in instance_candidates:
                ...
                self.root.after(
                    0,
                    lambda size=instance_type: self.status_var.set(
                        f"Ensuring Azure batch compute ({size})..."
                    ),
                )
                try:
                    compute = AmlCompute(
